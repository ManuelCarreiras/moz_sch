...
class TestStudentAssignment(unittest.TestCase):
    ...
    def test_get_student_assignments(self):
        ...
        response = self.client.get("/student/assignments/{}".format(self.student_id),
                                   headers={"Authorization": API_KEY})
        # Should return 200 even if no assignments found
        self.assertIn(response.status_code, [200, 401, 403])
        if response.status_code == 200:
            res_answer = json.loads(response.get_data())
            self.assertIn("assignments", res_answer)

    def test_get_student_assignments_invalid(self):
        """Test getting assignments for invalid student"""
        wrong_id = str(uuid.uuid4())
        response = self.client.get("/student/assignments/{}".format(wrong_id),
                                   headers={"Authorization": API_KEY})
